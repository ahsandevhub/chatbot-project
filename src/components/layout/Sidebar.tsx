...
const Sidebar: React.FC<SidebarProps> = ({ isSidebarOpen, toggleSidebar }) => {
  const { conversations, currentConversationId, setCurrentConversationId, addConversation, deleteConversation, renameConversation } = useChat();
    ...
    >
            ...
            {Object.entries(groupedConversations).map(([group, groupConversations]) => (
              groupConversations.length > 0 && (
                <div key={group} className="space-y-1">
                  <h3 className="px-3 text-xs font-medium text-muted-foreground mb-2">{group}</h3>
                  <div className="space-y-1">
                    {groupConversations.map(conversation => (
                      <div
                        ...
                        onMouseLeave={() => setHoveredChat(conversation.id === editingConversationId ? conversation.id : null)}
                      >
                              ...
                              <DropdownMenuTrigger asChild>
                                <button 
                                  className={`p-1 hover:bg-accent rounded ${
                                    hoveredChat === conversation.id ? "opacity-100" : "opacity-0"
      ...
      <div className="flex-shrink-0 p-3 border-t border-border">
        <button 
          ...
          className={`w-full flex items-center justify-center gap-2 rounded-md ${theme === 'dark' ? 'bg-white hover:bg-white/90 text-gray-900' : 'bg-gray-900 hover:bg-gray-800 text-white'} transition-colors duration-200 py-2 px-3 text-sm`}
        >
